...
def aggregate_snp_by_celltype(node_mask, labels, label_names, snp_names=None):
    ...
    if snp_names is not None:
        df.columns = snp_names
    else:
        df.columns = [f"SNP_{i}" for i in range(df.shape[1])]
...
def run_snp_celltype_score(
    ...
    if snp_name_file is not None:
        snp_names = pd.read_csv(snp_name_file, header=None)[0].astype(str).tolist()
    else:
        snp_names = None

    print("[OK] Input files loaded.")
    print(f"[INFO] SNP x Cell matrix shape: {data.shape}")
    ...
    print("[7/7] Calculating SNP importance score in each Celltype...")

    snp_celltype_score = aggregate_snp_by_celltype(
    ...
    snp_score_path = os.path.join(outdir, "SNP_in_Celltype_score.csv")
    snp_celltype_score.to_csv(snp_score_path)

    print("[OK] SNP importance scoring completed.")
    print(f"[OK] SNP in Celltype score saved to: {snp_score_path}")
    ...
    return {
        ...
        "snp_celltype_score": snp_celltype_score,
